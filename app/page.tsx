    ...
    <main className={"midas-app " + theme}>
      ...
      <Tabs value={tab} onValueChange={setTab} className="workspace">
        ...
        <div className="budget-workspace">
          ...
          <BudgetPeriodSettings key={monthKey} error={error} period={data.period} profile={data.budgetProfile} transactions={data.transactions} theme={theme} disabled={saving || sheetLoading || refreshing} onSave={payload => mutate(payload, "Presupuesto del periodo guardado")} />
        ...
        <TabsContent value="dashboard" className="page-content">
          ...
          {!metrics.setupComplete && <section className="setup-banner"><div className="setup-icon"><Sparkles /></div><div><strong>Define tu presupuesto</strong><p>Completa ingreso, ahorro y presupuestos para activar el MIDAS Score y el forecast.</p></div><Button className="gold-button" onClick={() => setTab("plan")}>Configurar <ChevronRight /></Button></section>}
        ...
        <TabsContent value="plan" className="page-content">
          ...
          <section className="panel budget-table-panel">
            ...
            {!metrics.categoryRows.length && <EmptyState icon={<Target />} title="No hay gastos programados guardados" text="Agrega manualmente una categoría y su monto para este periodo con el botón Categoría." />}
          </section>
        </TabsContent>
