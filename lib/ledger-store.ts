...
export async function withMovementCode<T>(tables: MidasTables, userId: string, date: string, type: string,
  ...
  try {
    await createRow(tables, TABLES.sequences, sequenceId, { user_id: userId, period: date.slice(0, 7), kind: prefix.slice(-1), last_number: 0 });
  } catch (error) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    try {
      const counter = await tables.incrementRowColumn({ databaseId: DATABASE_ID, tableId: TABLES.sequences,
...
export async function ensureMovementCodes(tables: MidasTables, userId: string, rows: MidasRow[]) {
  ...
  for (const row of missing) {
    ...
    row.midas_code = await withMovementCode(tables, userId, String(row.date), String(row.type), async (code, transactionId) => {
      const current = await tables.getRow<MidasRow>({ databaseId: DATABASE_ID, tableId: TABLES.transactions, rowId: row.$id, transactionId });
      if (current.user_id !== userId) throw new Error("El movimiento no existe.");
      if (current.midas_code) return current.midas_code;
      await updateRow(tables, TABLES.transactions, row.$id, { midas_code: code }, transactionId);
      return code;
    });
  }
  return rows;
