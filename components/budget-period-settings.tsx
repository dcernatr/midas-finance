...
import { type BudgetCategory, type BudgetProfile, type BudgetPeriod, initialPlanReview, payrollCandidates } from "@/lib/budgeting";

export function BudgetPeriodSettings({ period, profile, categories = [], transactions, onSave, disabled, theme, error }: {
  ...
  const review = initialPlanReview(categories, profile);
  ...
  return <section className="period-settings panel">
    ...
    {period.key === "2026-09" && <div className={"initial-budget-status " + (profile.initialApplied ? "applied" : "pending")} role="status">
      <div><strong>{profile.initialApplied ? "Presupuesto inicial guardado en tu cuenta" : "El presupuesto inicial todavía no está cargado"}</strong><p>{profile.initialApplied ? "Los cambios posteriores se conservan. Dashboard y Programados usan los importes actuales." : "9 conceptos · S/ 13,530 · Inicio 28/08/2026. Revisa y guarda para incorporarlos a tus datos."}</p></div>
      {!profile.initialApplied && <Button className="gold-button" disabled={disabled} onClick={() => setInitial(true)}>Revisar y cargar programados</Button>}
    </div>}
    ...
    <Dialog open={initial} onOpenChange={v => { if (!disabled) setInitial(v); }}><DialogContent className="midas-dialog budget-dialog" data-theme={theme}>
      ...
      <div className="initial-plan">{review.rows.map(c => <div key={c.name}><span style={{ color: c.color }}>{c.name}<small>{c.conflict ? c.conflict + (c.current !== null ? ": S/ " + c.current : "") : c.missing ? "Se agregará" : "Ya guardado: se conserva"}</small></span><strong>S/ {c.budget.toLocaleString("es-PE")}</strong></div>)}<div><strong>Total del plan inicial</strong><strong>S/ 13,530</strong></div></div>
      {review.extras.length > 0 && <p>También existen {review.extras.length} categorías adicionales: se conservarán y se sumarán al presupuesto total.</p>}
      {(review.conflicts.length > 0 || review.dateConflict) && <p role="alert">Hay diferencias con tus datos actuales. Revisa {review.dateConflict ? "la fecha de inicio" : "los importes señalados en Gastos Programados"} antes de cargar; no se reemplazará nada automáticamente.</p>}
      {error && <p role="alert" className="category-pending">{error}</p>}
      <Button className="gold-button" disabled={disabled || review.conflicts.length > 0 || review.dateConflict} onClick={async () => { if (await onSave({ action: "budget_initial" })) setInitial(false); }}>{disabled ? "Guardando…" : "Guardar programados en mi cuenta"}</Button>
    </DialogContent></Dialog>
  </section>;
}
